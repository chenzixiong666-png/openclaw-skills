...
Default target categories (unless user explicitly overrides):
- 固件缺陷
- RN缺陷
- 后台缺陷
- 需求缺陷

Do NOT include 硬件缺陷 or 结构缺陷 in the weekly report unless the user explicitly asks for them. These categories are managed separately and should be excluded from:
- 缺陷看板 totals and listings
- 严重缺陷 section
- 风险与需求 statistics
- All priority/status distribution counts

Other possible categories (only if user requests):
- Android缺陷
- iOS缺陷
- 硬件缺陷
- 结构缺陷
...
- prefer 严重 / 最高 / 较高 priority items
- include unresolved or regression-related items first
- if too many, shortlist the most report-worthy issues

### 5a. Status filter (default reporting scope)

By default, only count defects in these statuses:
- 测试提交
- 待回归
- 回归不通过
- 下一版本修复
...
Exclude from counts: 回归通过 / 关闭 / 非bug / 待评审 / 持续跟踪 / 已确认 / 修复中

This means the weekly report reflects the **active unresolved defect backlog**, not historical totals.
...
- only keep the requested target categories; default to：固件缺陷 / RN缺陷 / 后台缺陷 / 需求缺陷 (NEVER include 硬件缺陷 / 结构缺陷 unless explicitly requested)
